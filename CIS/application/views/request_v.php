    <div id="request-modal" tabindex="-1" data-modal-backdrop="static" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-xl max-w-7xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <!-- Modal footer -->
                <div class="inline-flex items-right p-4 md:p-5 space-x-3 rtl:space-x-reverse border-t border-gray-200 rounded-b dark:border-gray-700">
                    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" id="btnSend"><i class="fa-solid fa-paper-plane"></i> Send Request</button>
                    <button data-modal-hide="request-modal" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-orange-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-orange-100 dark:focus:ring-gray-700 dark:bg-orange-800 dark:text-gray-400 dark:border-orange-600 dark:hover:text-white dark:hover:bg-gray-700">Decline</button>
                </div>
            </div>
        </div>
    </div>
    <script>
        $(document).ready(function(){
            ...
            function requestExist_v(){
                $.ajax({
                    url:"Create/requestExist_c",
                    method:"POST",
                    data:$("#frmInputs").serialize(),
                    dataType:"json",
                    success:function(response){
                        if(response.success){
                            alert("Request No. Already Exist");
